...
public class Main {
    ...
    private static int solution(String str, String specificStr) {
        ...
        char[] specificStrChar = specificStr.toCharArray();
        int count = 0;
        int j = 0;
        int countExist = 0;
        ...
        for(int i = 0; i < str.length(); i++) {
            j = 0;
            countExist = 0;
            if(strChar[i] == specificStrChar[j]){
                countExist++;
                int tmp = i+1;
                if(tmp == str.length())continue;
                for(j = 1; j < specificStrChar.length; j++) {
                    
                    if(strChar[tmp] == specificStrChar[j]){
                        countExist++;
                        tmp++;
                    }
                    else break;
                }
                if(countExist == specificStr.length())count++;
            }
        }
    ...
    // private static int solution2(String str, String specificStr) {
    //     int count = 0;
